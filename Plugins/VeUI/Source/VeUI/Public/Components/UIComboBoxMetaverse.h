...
#include "UIComboBoxMetaverse.generated.h"

/**
 * The combobox allows you to display a list of options to the user in a dropdown menu for them to select one.
 */
UCLASS(meta=( DisplayName="ComboBox (Metaverse)"))
class VEUI_API UUIComboBoxMetaverse : public UWidget
{
	GENERATED_UCLASS_BODY()

	DECLARE_DYNAMIC_MULTICAST_DELEGATE_TwoParams(FOnSelectionChangedEvent, FVePackageMetadata, SelectedItem, ESelectInfo::Type, SelectionType);
...
private:
	...
	TArray<FVePackageMetadata> DefaultOptions;

	/** The item in the combobox to select by default */
	UPROPERTY(EditAnywhere, Category=Content)
	FVePackageMetadata SelectedOption;
...
public:
	...
	void AddOption(const FVePackageMetadata& Option);
	...
	UFUNCTION(BlueprintCallable, Category="ComboBox")
	FVePackageMetadata GetOptionAtIndex(int32 Index) const;
	...
	UFUNCTION(BlueprintCallable, Category="ComboBox")
	FVePackageMetadata GetSelectedOption() const;
...
protected:
	...
	void UpdateOrGenerateWidget(TSharedPtr<FVePackageMetadata> Item);

	/** Called by slate when it needs to generate a new item for the combobox */
	virtual TSharedRef<SWidget> HandleGenerateWidget(TSharedPtr<FVePackageMetadata> Item) const;

	/** Called by slate when the underlying combobox selection changes */
	virtual void HandleSelectionChanged(TSharedPtr<FVePackageMetadata> Item, ESelectInfo::Type SelectionType);
...
protected:
	...
	TArray< TSharedPtr<FVePackageMetadata> > Options;

	/** A shared pointer to the underlying slate combobox */
	TSharedPtr< SComboBox< TSharedPtr<FVePackageMetadata> > > MyComboBox;
	...
	TSharedPtr<FVePackageMetadata> CurrentOptionPtr;
};
