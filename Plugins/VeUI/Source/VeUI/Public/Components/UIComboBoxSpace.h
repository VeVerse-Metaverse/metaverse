...
#include "UIComboBoxSpace.generated.h"

/**
 * The combobox allows you to display a list of options to the user in a dropdown menu for them to select one.
 */
UCLASS(meta=( DisplayName="ComboBox (World)"))
class VEUI_API UUIComboBoxSpace : public UWidget {
	GENERATED_UCLASS_BODY()
	DECLARE_DYNAMIC_MULTICAST_DELEGATE_TwoParams(FOnSelectionChangedEvent, FVeWorldMetadata, SelectedItem, ESelectInfo::Type, SelectionType);
...
private:
	...
	TArray<FVeWorldMetadata> DefaultOptions;

	/** The item in the combobox to select by default */
	UPROPERTY(EditAnywhere, Category=Content)
	FVeWorldMetadata SelectedOption;
...
public:
	...
	void AddOption(const FVeWorldMetadata& Option);
	...
	UFUNCTION(BlueprintCallable, Category="ComboBox")
	FVeWorldMetadata GetOptionAtIndex(int32 Index) const;
	...
	UFUNCTION(BlueprintCallable, Category="ComboBox")
	FVeWorldMetadata GetSelectedOption() const;
...
protected:
	...
	void UpdateOrGenerateWidget(TSharedPtr<FVeWorldMetadata> Item);

	/** Called by slate when it needs to generate a new item for the combobox */
	virtual TSharedRef<SWidget> HandleGenerateWidget(TSharedPtr<FVeWorldMetadata> Item) const;

	/** Called by slate when the underlying combobox selection changes */
	virtual void HandleSelectionChanged(TSharedPtr<FVeWorldMetadata> Item, ESelectInfo::Type SelectionType);
...
protected:
	...
	TArray<TSharedPtr<FVeWorldMetadata>> Options;

	/** A shared pointer to the underlying slate combobox */
	TSharedPtr<SComboBox<TSharedPtr<FVeWorldMetadata>>> MyComboBox;
	...
	TSharedPtr<FVeWorldMetadata> CurrentOptionPtr;
};
