...
#include "UIComboBoxUser.generated.h"

/**
 * The combobox allows you to display a list of options to the user in a dropdown menu for them to select one.
 */
UCLASS(meta=( DisplayName="ComboBox (User)"))
class VEUI_API UUIComboBoxUser : public UWidget
{
	GENERATED_UCLASS_BODY()

	DECLARE_DYNAMIC_MULTICAST_DELEGATE_TwoParams(FOnSelectionChangedEvent, FVeUserMetadata, SelectedItem, ESelectInfo::Type, SelectionType);
...
private:
	...
	TArray<FVeUserMetadata> DefaultOptions;

	/** The item in the combobox to select by default */
	UPROPERTY(EditAnywhere, Category=Content)
	FVeUserMetadata SelectedOption;
...
public:
	...
	void AddOption(const FVeUserMetadata& Option);
	...
	UFUNCTION(BlueprintCallable, Category="ComboBox")
	FVeUserMetadata GetOptionAtIndex(int32 Index) const;
	...
	UFUNCTION(BlueprintCallable, Category="ComboBox")
	FVeUserMetadata GetSelectedOption() const;
...
protected:
	...
	void UpdateOrGenerateWidget(TSharedPtr<FVeUserMetadata> Item);

	/** Called by slate when it needs to generate a new item for the combobox */
	virtual TSharedRef<SWidget> HandleGenerateWidget(TSharedPtr<FVeUserMetadata> Item) const;

	/** Called by slate when the underlying combobox selection changes */
	virtual void HandleSelectionChanged(TSharedPtr<FVeUserMetadata> Item, ESelectInfo::Type SelectionType);
...
protected:
	...
	TArray< TSharedPtr<FVeUserMetadata> > Options;

	/** A shared pointer to the underlying slate combobox */
	TSharedPtr< SComboBox< TSharedPtr<FVeUserMetadata> > > MyComboBox;
	...
	TSharedPtr<FVeUserMetadata> CurrentOptionPtr;
};
