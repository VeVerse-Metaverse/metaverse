...
/**
 * Test download of same files requested multiple times. Each request must get its own complete callback downloading each file only once.
 */
IMPLEMENT_SIMPLE_AUTOMATION_TEST(FSameDownloadTest, "Game.VeDownload.SameDownloadTest", EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)

class FTestSameDownloadLatentCommand final : public IVeBaseLatentCommand {
...
public:
	explicit FTestSameDownloadLatentCommand(FAutomationTestBase* InTest, IVeBaseLatentCommand* InParentCommand = nullptr, const double InTimeout = 30.0)
	...
	virtual ~FTestSameDownloadLatentCommand() override { }
};

bool FSameDownloadTest::RunTest(const FString& Parameters) {
	constexpr float LatentCommandTimeout = 100.0f;

	FAutomationTestFramework::Get().EnqueueLatentCommand(MakeShared<FTestSameDownloadLatentCommand>(this, nullptr, LatentCommandTimeout));
