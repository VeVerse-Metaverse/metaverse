...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(FPreSignedUrlDownloadTest, "Game.VeDownload.PreSignedUrlDownloadTest", EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)

class FTestPreSignedUrlDownloadLatentCommand final : public IVeBaseLatentCommand {
...
public:
	explicit FTestPreSignedUrlDownloadLatentCommand(FAutomationTestBase* InTest, IVeBaseLatentCommand* InParentCommand = nullptr, const double InTimeout = 30.0)
		: IVeBaseLatentCommand(InTest, InParentCommand, InTimeout) { }

	virtual ~FTestPreSignedUrlDownloadLatentCommand() override { }
	...
	virtual void Run() override {
		...
		for (const auto& RequestUrl : TestUrls) {
			...
			RequestMetadata.bUsePreSignedUrl = true;
...
bool FPreSignedUrlDownloadTest::RunTest(const FString& Parameters) {
	constexpr float LatentCommandTimeout = 3600.0f;

	FAutomationTestFramework::Get().EnqueueLatentCommand(MakeShared<FTestPreSignedUrlDownloadLatentCommand>(this, nullptr, LatentCommandTimeout));
