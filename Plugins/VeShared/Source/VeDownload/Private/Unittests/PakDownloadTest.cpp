...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(FPakDownloadTest, "Game.VeDownload.PakDownloadTest", EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)

class FTestPakDownloadLatentCommand final : public IVeBaseLatentCommand {
...
public:
	explicit FTestPakDownloadLatentCommand(FAutomationTestBase* InTest, IVeBaseLatentCommand* InParentCommand = nullptr, const double InTimeout = 30.0)
		: IVeBaseLatentCommand(InTest, InParentCommand, InTimeout) { }

	virtual ~FTestPakDownloadLatentCommand() override { }
...
bool FPakDownloadTest::RunTest(const FString& Parameters) {
	constexpr float LatentCommandTimeout = 3600.0f;

	FAutomationTestFramework::Get().EnqueueLatentCommand(MakeShared<FTestPakDownloadLatentCommand>(this, nullptr, LatentCommandTimeout));
