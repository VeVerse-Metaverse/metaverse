...
UCLASS()
class VEGAMEFRAMEWORK_API AGizmoBase : public AActor {
	GENERATED_BODY()
...
public:
	...
	virtual ETransformType GetGizmoType() const { return ETransformType::None; }
	...
	virtual FTransform GetDeltaTransform(const FVector& LookingVector, const FVector& RayStartPoint, const FVector& RayEndPoint, ETransformationDomain Domain);
	 ...
	 * @param ReferenceLocation FVector - The Location of where the Gizmo is seen (i.e. Camera Location)
	 * @param ReferenceLookDirection - the direction the reference is looking (i.e. Camera Look Direction)
	 * @param FieldOfView - Field of View of Camera, in Degrees
	*/
	void ScaleGizmo(const FVector& ReferenceLocation, const FVector& ReferenceLookDirection, float FieldOfView = 90.f);

	UFUNCTION(BlueprintCallable, Category = "Gizmo")
	ETransformationDomain GetTransformationAxes(class USceneComponent* ComponentHit) const;
...
public:
	...
	DECLARE_EVENT_ThreeParams(AGizmoBase, FOnGizmoStateChanged, ETransformType /*GizmoType*/, bool /*bTransformInProgress*/, ETransformationDomain /*CurrentDomain*/);
...
public:
	...
	void OnGizmoStateChangedBP(ETransformType InGizmoType, bool bInTransformInProgress, ETransformationDomain CurrentDomain);
...
protected:
	...
	*/
	...
	class USceneComponent* ScalingSceneComponent;
