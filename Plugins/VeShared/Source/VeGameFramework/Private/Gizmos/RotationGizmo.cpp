...
FTransform ARotationGizmo::GetDeltaTransform(const FVector& LookingVector, const FVector& RayStartPoint, const FVector& RayEndPoint, ETransformationDomain Domain) {
	...
	if (AreRaysValid()) {
		...
		float factor = (FVector::DotProduct(FVector::CrossProduct(deltaLocation, prevDeltaLocation), planeNormal)) >= 0.f ? -1.f : 1.f;
...
FTransform ARotationGizmo::GetSnappedTransform(FTransform& outCurrentAccumulatedTransform, const FTransform& DeltaTransform, ETransformationDomain Domain, float SnappingValue) const {
