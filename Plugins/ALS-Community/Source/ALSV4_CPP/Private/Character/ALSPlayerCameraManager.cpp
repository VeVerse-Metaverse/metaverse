...
const FName NAME_OverrideRotation(TEXT("Override_Rotation"));
...
bool AALSPlayerCameraManager::CustomCameraBehavior(float DeltaTime, FVector& Location, FRotator& Rotation, float& FOV)
{
	...
	FRotator OverrideCameraRotation = UKismetMathLibrary::RLerp(GetOwningPlayerController()->GetControlRotation(),
																OverrideViewRotation,
																GetCameraBehaviorParam(NAME_OverrideRotation), true);
	GetOwningPlayerController()->SetControlRotation(OverrideCameraRotation);
