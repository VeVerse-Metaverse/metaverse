...
void AALSBaseCharacter::BeginPlay() {
	...
	if (IsLocallyControlled() && GetNetMode() != NM_Standalone) {
		...
		if (IsValid(World)) {
			...
			// Make character sprint at the start of the game after 3 seconds.
			HackGait_TimerDelegate.BindWeakLambda(this, [this] {
				// Switch to sprint gait.
				SprintAction_Implementation(true);
				auto* World = GetWorld();
				if (!IsValid(World)) {
					return;
				}
				// Set timer to switch back to running gait.
				World->GetTimerManager().SetTimer(HackGait_TimerHandle2, HackGait_TimerDelegate2, 1.0f, false);
				World->GetTimerManager().ClearTimer(HackGait_TimerHandle);
				// Cleanup.
				//HackGait_TimerDelegate.Unbind();
			});

			// Make character stop sprinting after 1 second after previous timer.
			HackGait_TimerDelegate2.BindWeakLambda(this, [this] {
				// Switch back to running gait.
				SprintAction_Implementation(false);
				auto* World = GetWorld();
				if (!IsValid(World)) {
					return;
				}
				// Clear timer.
				World->GetTimerManager().ClearTimer(HackGait_TimerHandle2);
				// Cleanup.
				//HackGait_TimerDelegate2.Unbind();
			});

			World->GetTimerManager().SetTimer(HackGait_TimerHandle, HackGait_TimerDelegate, 3.0f, false);
		}
	}
...
void AALSBaseCharacter::RagdollStart() {
	...
	TargetRagdollLocation = GetMesh()->GetSocketLocation(PelvisSocketName);
	ServerRagdollPull = 0;
	...
	GetMesh()->SetAllBodiesBelowSimulatePhysics(PelvisSocketName, true, true);
...
void AALSBaseCharacter::RagdollEnd() {
	...
	if (GetMesh()->GetAnimInstance()) {
		GetMesh()->GetAnimInstance()->SavePoseSnapshot(RagdollPoseName);
	}
...
void AALSBaseCharacter::SetMovementModel() {
	...
	if (MovementModel.IsNull()) {
		ensureMsgf(!MovementModel.IsNull(), TEXT("MovementModel is null"));
		return;
	}
...
void AALSBaseCharacter::RagdollUpdate(float DeltaTime) {
	...
	const FVector NewRagdollVel = GetMesh()->GetPhysicsLinearVelocity(RootBoneName);
...
void AALSBaseCharacter::SetActorLocationDuringRagdoll(float DeltaTime) {
	...
	const FRotator PelvisRot = GetMesh()->GetSocketRotation(PelvisSocketName);
	...
	if (!IsLocallyControlled()) {
		...
		FName RagdollSocketPullName = RagdollSpeed > 300 ? LowerSpineBoneName : PelvisBoneName;
		GetMesh()->AddForce(
		...
		if (bCanUpdateMovingRot) {
			...
			if (RotationMode == EALSRotationMode::VelocityDirection) {
				...
				if (Gait == EALSGait::Sprinting) {
					...
					// Walking or Running..
					const float YawOffsetCurveVal = GetAnimCurveValue(YawOffsetCurveName);
			...
			const float RotAmountCurve = GetAnimCurveValue(RotationAmountCurveName);
