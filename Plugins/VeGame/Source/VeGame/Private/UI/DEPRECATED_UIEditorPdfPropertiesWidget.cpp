...
#include "UI/DEPRECATED_UIEditorPdfPropertiesWidget.h"
...
void UDEPRECATED_UIEditorPdfPropertiesWidget::NativeOnTextureLoaded(const FTextureLoadResult& InResult) {
...
void UDEPRECATED_UIEditorPdfPropertiesWidget::NativeOnInitialized() {
	...
	if (IsValid(FileBrowserButtonWidget)) {
		FileBrowserButtonWidget->GetOnButtonClicked().AddUObject(this, &UDEPRECATED_UIEditorPdfPropertiesWidget::OnFileBrowserButtonClicked);
	}

	Super::NativeOnInitialized();
}

void UDEPRECATED_UIEditorPdfPropertiesWidget::OnUrlTextCommittedCallback(const FText& InText, ETextCommit::Type /*InCommitMethod*/) {
	if (!InText.IsEmpty()) {
		Url = InText.ToString();
...
void UDEPRECATED_UIEditorPdfPropertiesWidget::OnFileBrowserButtonClicked() {
#if PLATFORM_DESKTOP
...
void UDEPRECATED_UIEditorPdfPropertiesWidget::SetUrl(const FString& InUrl) {
	Url = InUrl;
...
FString UDEPRECATED_UIEditorPdfPropertiesWidget::GetUrl() {
	return Url;
}
